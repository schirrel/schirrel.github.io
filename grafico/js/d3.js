(function() {
    ...
    function d3_dispatch(type) {
        ...
        dispatch.add = function(listener) {
            for (var i = 0; i < listeners.length; i++) {
                if (listeners[i].listener == listener) return dispatch;
            }
            listeners.push({
                listener: listener,
                on: true
    ...
    d3.rgb = function(r, g, b) {
        return arguments.length === 1 ? d3_rgb_parse("" + r, d3_rgb, d3_hsl_rgb) : d3_rgb(~~r, ~~g, ~~b);
    };
    ...
    d3_Rgb.prototype.darker = function(k) {
        k = Math.pow(0.7, arguments.length ? k : 1);
        return d3_rgb(Math.max(0, Math.floor(k * this.r)), Math.max(0, Math.floor(k * this.g)), Math.max(0, Math.floor(k * this.b)));
    };
    ...
    function d3_rgb_parse(format, rgb, hsl) {
        ...
        if (m1) {
            ...
            switch (m1[1]) {
                case "hsl":
                    {
                        return hsl(parseFloat(m2[0]), parseFloat(m2[1]) / 100, // percentage
    ...
    for (var d3_rgb_name in d3_rgb_names) {
        d3_rgb_names[d3_rgb_name] = d3_rgb_parse(d3_rgb_names[d3_rgb_name], d3_rgb, d3_hsl_rgb);
    }
    d3.hsl = function(h, s, l) {
        return arguments.length === 1 ? d3_rgb_parse("" + h, d3_rgb_hsl, d3_hsl) : d3_hsl(+h, +s, +l);
    };
    ...
    d3_selectionPrototype.insert = function(name, before) {
        ...
        function insert() {
            return this.insertBefore(document.createElement(name), d3_select(before, this));
        }
    ...
    d3.selectAll = function(selector) {
        return typeof selector === "string" ? d3_selectionRoot.selectAll(selector) : d3_selection([selector]);
    };
    ...
    d3_transitionPrototype.delay = function(value) {
        var groups = this;
        return groups.each(typeof value === "function" ? function(d, i, j) {
    ...
    function d3_scale_ordinal(domain, ranger) {
        ...
        scale.domain = function(x) {
            ...
            var i = -1,
                ...
                xi;
            while (++i < n)
                if (!index[xi = x[i]]) index[xi] = domain.push(xi);
    ...
    d3.svg.arc = function() {
        ...
        function arc() {
            var r0 = innerRadius.apply(this, arguments),
                ...
                c1 = Math.cos(a1),
                s1 = Math.sin(a1);
            return da >= d3_svg_arcMax ? (r0 ? "M0," + r1 +
                "A" + r1 + "," + r1 + " 0 1,1 0," + (-r1) +
                "A" + r1 + "," + r1 + " 0 1,1 0," + r1 +
                "M0," + r0 +
                "A" + r0 + "," + r0 + " 0 1,0 0," + (-r0) +
        ...
        function axis(selection) {
            selection.each(function(d, i, j) {
                ...
                switch (orient) {
                    case "bottom":
                        {
                            tickTransform = d3_svg_axisX;subtickUpdate.attr("y2", tickMinorSize);tickEnter.select("text").attr("dy", ".71em").attr("text-anchor", "middle");tickUpdate.select("line").attr("y2", tickMajorSize);tickUpdate.select("text").attr("y", Math.max(tickMajorSize, 0) + tickPadding);pathUpdate.attr("d", "M" + range[0] + "," + tickEndSize + "V0H" + range[1] + "V" + tickEndSize);
                            break;
                        }
                    case "top":
                        {
                            tickTransform = d3_svg_axisX;subtickUpdate.attr("y2", -tickMinorSize);tickEnter.select("text").attr("text-anchor", "middle");tickUpdate.select("line").attr("y2", -tickMajorSize);tickUpdate.select("text").attr("y", -(Math.max(tickMajorSize, 0) + tickPadding));pathUpdate.attr("d", "M" + range[0] + "," + -tickEndSize + "V0H" + range[1] + "V" + -tickEndSize);
                            break;
                        }
                    case "left":
                        {
                            tickTransform = d3_svg_axisY;subtickUpdate.attr("x2", -tickMinorSize);tickEnter.select("text").attr("dy", ".32em").attr("text-anchor", "end");tickUpdate.select("line").attr("x2", -tickMajorSize);tickUpdate.select("text").attr("x", -(Math.max(tickMajorSize, 0) + tickPadding));pathUpdate.attr("d", "M" + -tickEndSize + "," + range[0] + "H0V" + range[1] + "H" + -tickEndSize);
                            break;
                        }
                    case "right":
                        {
                            tickTransform = d3_svg_axisY;subtickUpdate.attr("x2", tickMinorSize);tickEnter.select("text").attr("dy", ".32em");tickUpdate.select("line").attr("x2", tickMajorSize);tickUpdate.select("text").attr("x", Math.max(tickMajorSize, 0) + tickPadding);pathUpdate.attr("d", "M" + tickEndSize + "," + range[0] + "H0V" + range[1] + "H" + tickEndSize);
                            break;
                        }
                }
                ...
                function transition(o) {
                    return selection.delay ? o.transition().delay(selection[j][i].delay).duration(selection[j][i].duration).ease(selection.ease()) : o;
                }
            });
        }
    ...
    var d3_behavior_dragEvent, d3_behavior_dragTarget, d3_behavior_dragArguments, d3_behavior_dragOffset, d3_behavior_dragMoved, d3_behavior_dragStopClick;
    ...
    var d3_behavior_zoomDiv, d3_behavior_zoomPanning, d3_behavior_zoomZooming, d3_behavior_zoomLocations = {},
        ...
        d3_behavior_zoomXyz, d3_behavior_zoomDispatch, d3_behavior_zoomTarget, d3_behavior_zoomArguments, d3_behavior_zoomMoved, d3_behavior_zoomStopClick;

    function d3_behavior_zoomLocation(point) {
        return [point[0] - d3_behavior_zoomXyz[0], point[1] - d3_behavior_zoomXyz[1], d3_behavior_zoomXyz[2]];
    }
    ...
    function d3_behavior_zoomTouchmove() {
        ...
        switch (touches.length) {
            case 1:
                {
                    var touch = touches[0];d3_behavior_zoomTo(d3_behavior_zoomXyz[2], touch, d3_behavior_zoomLocations[touch.identifier]);
                    break;
                }
            case 2:
                {
